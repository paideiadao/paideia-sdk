    ...
    test("Create proposal") {
        ...
        ergoClient.execute(new java.util.function.Function[BlockchainContext,Unit] {
            override def apply(_ctx: BlockchainContext): Unit = {
                ...
                config.set(ConfKeys.im_paideia_contracts_proposal(proposalContract.ergoTree.bytes),proposalContract.contractSignature)
                val proposalBox = proposalContract.box(ctx,0,Array(0L,0L),0L,ctx.createPreHeader().build().getTimestamp()+3600000,-1.toShort)

                val actionContract = ActionSendFundsBasic(PaideiaContractSignature(daoKey=dao.key))
                config.set(ConfKeys.im_paideia_contracts_action(actionContract.ergoTree.bytes),actionContract.contractSignature)
                ...
                val createProposalContract = CreateProposal(PaideiaContractSignature(daoKey=dao.key))
                val createProposalBox = createProposalContract.box(ctx,proposalBox.box,Array(actionBox.box,actionBox.box),stakeKey,dummyAddress)

                val dummyTx = (new ErgoTransaction()).addOutputsItem(createProposalBox.ergoTransactionOutput())
