    ...
    test("Send funds") {
        ...
        ergoClient.execute(new java.util.function.Function[BlockchainContext,Unit] {
            override def apply(_ctx: BlockchainContext): Unit = {
                ...
                treasuryContract.newBox(treasuryContract.box(ctx,dao.config,233000000L,List(new ErgoToken(Util.randomKey,20L))).inputBox(),false)
                treasuryContract.newBox(treasuryContract.box(ctx,dao.config,2000000L,List[ErgoToken]()).inputBox(),false)
                treasuryContract.newBox(treasuryContract.box(ctx,dao.config,233000000L,List(new ErgoToken(testToken,20L))).inputBox(),false)
                treasuryContract.newBox(treasuryContract.box(ctx,dao.config,2000000L,List(new ErgoToken(testToken,20L),new ErgoToken(Util.randomKey,10L))).inputBox(),false)
                config.set(ConfKeys.im_paideia_contracts_treasury,treasuryContract.contractSignature)
                dao.proposals(0) = new Proposal()

                val proposalContract = ProposalBasic(PaideiaContractSignature(daoKey=dao.key))
                val proposalBox = proposalContract.box(ctx,0,Array(0L,100L),0L,ctx.createPreHeader().build().getTimestamp()-3600000,1.toShort)
                ...
                val actionContract = ActionSendFundsBasic(PaideiaContractSignature(daoKey=dao.key))
                val actionBox = actionContract.box(ctx,0,1,ctx.createPreHeader().build().getTimestamp()-3600000,
                    Array(
                        CostingBox(false,ctx.newTxBuilder().outBoxBuilder()
                        .contract(dummyAddress.toErgoContract())
                        .tokens(new ErgoToken(testToken,30L))
                        .value(468000000L)
                        .build().convertToInputWith(Util.randomKey,0.toShort).asInstanceOf[InputBoxImpl].getErgoBox())
                    )
                )
