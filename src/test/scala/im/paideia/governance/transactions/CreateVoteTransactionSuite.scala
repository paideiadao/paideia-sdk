  ...
  test("Create Vote") {
    ...
    ergoClient.execute(new java.util.function.Function[BlockchainContext, Unit] {
      override def apply(_ctx: BlockchainContext): Unit = {
        ...
        val createVoteProxyContract =
          CreateVoteProxy(PaideiaContractSignature(daoKey = dao.key))
        val createVoteBox =
          createVoteProxyContract.box(ctx, stakeKey, dummyAddress).ergoTransactionOutput()
