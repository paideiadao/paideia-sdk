    ...
    test("Profit share erg tx on empty state") {
        ...
        ergoClient.execute(new java.util.function.Function[BlockchainContext,Unit] {
            override def apply(_ctx: BlockchainContext): Unit = {
                ...
                val dummyBlock = (new FullBlock()).header(new BlockHeader().timestamp(state.nextEmission-1000L).height(ctx.getHeight()+30)).blockTransactions(new BlockTransactions().transactions(new Transactions()))
    ...
    test("Profit share staked token tx on empty state") {
        ...
        ergoClient.execute(new java.util.function.Function[BlockchainContext,Unit] {
            override def apply(_ctx: BlockchainContext): Unit = {
                ...
                dao.config.set(ConfKeys.im_paideia_staking_profit_tokenids, Array(ErgoId.create(sigUsd).getBytes()))
                ...
                val splitProfitContract = SplitProfit(PaideiaContractSignature(daoKey=dao.key))
                val splitProfitBox = splitProfitContract.box(ctx, 2000000L, List[ErgoToken](new ErgoToken(dao.config.getArray[Byte](ConfKeys.im_paideia_dao_tokenid),1000000L)))
                splitProfitContract.newBox(splitProfitBox.inputBox(), false)
    ...
    test("Profit share whitelisted token tx on empty state") {
        ...
        ergoClient.execute(new java.util.function.Function[BlockchainContext,Unit] {
            override def apply(_ctx: BlockchainContext): Unit = {
                ...
                val splitProfitContract = SplitProfit(PaideiaContractSignature(daoKey=dao.key))
                val splitProfitBox = splitProfitContract.box(ctx, 2000000L, List[ErgoToken](new ErgoToken(sigUsd,1000000L)))
                splitProfitContract.newBox(splitProfitBox.inputBox(), false)
