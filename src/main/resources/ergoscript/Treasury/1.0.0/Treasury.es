 ...
 *
 * @param daoKeyId Token ID of the dao key
 * @param paideiaDaoKey Token ID of the paideia dao key
 * @param paideiaTokenId Token ID of Paideia token
 * @param daoActionTokenIdAndStakeStateTokenId Token ID of dao action token and stake state nft concatenated
 * 
 * @return
 */
@contract def treasury(daoKeyId: Coll[Byte], paideiaDaoKey: Coll[Byte], paideiaTokenId: Coll[Byte], daoActionTokenIdAndStakeStateTokenId: Coll[Byte]) = {
    ...
    //Treasury utxo should be updated in place if contract is changed or utxo too old
    ...
    //Spending from the treasury with a passed proposal/action
    ...
    //The transactions needed for staking rewards and profit distribution are funded by
    //the treasury and include fees paid to the paideia dao
    ...
    //Prevent dust in the treasury from becoming an issue
