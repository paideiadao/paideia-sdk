{
    ...
    val profit = plasmaStakingInput.R5[Coll[Long]].get.slice(5,plasmaStakingInput.R5[Coll[Long]].get.size)
    val totalStaked = plasmaStakingInput.R5[Coll[Long]].get(1)
    ...
    val tokensStaked = stakeAmount == (plasmaStakingOutput.tokens(1)._2 - plasmaStakingInput.tokens(1)._2) && stakeAmount == plasmaStakingOutput.R5[Coll[Long]].get(1) - totalStaked

    val singleStakeOp = stakeOperations.size == 1

    val correctNewState = stakeState.insert(stakeOperations, proof).get.digest == plasmaStakingOutput.R4[Coll[AvlTree]].get(0).digest
