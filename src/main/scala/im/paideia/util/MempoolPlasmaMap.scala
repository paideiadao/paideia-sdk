...
class MempoolPlasmaMap[K, V](
  ...
  /** Snapshot of the current mempool forks (digest -> in-progress tree), for callers that
    * need to carry pending, unconfirmed off-chain state across to a differently backed
    * map (e.g. StakingState.clone transplanting in-flight mempool chains onto a freshly
    * persisted snapshot). Independent copies, so mutating the result (or this map's own
    * forks afterwards) can't cross-contaminate the two maps.
    */
  ...
  /** Releases the underlying LevelDB handle (when storage is a VersionedLDBAVLStorage),
    * so the same directory can be reopened by a new store/map afterwards - e.g. to prove
    * a restore actually reads back from disk rather than from this instance's in-memory
    * state. VersionedLDBAVLStorage doesn't expose its LDBVersionedStore publicly, hence
    * the reflection. A no-op for any other storage implementation.
    */
  def close(): Unit = {
    storage match {
  ...
  /** Drains opQueue in FIFO order, applying every queued confirmed-height batch to
    ...
    * confirmed writes only ever land in the in-memory newlyConfirmedMap and opQueue grows
    * forever with nothing behind it on disk.
    *
    * A no-op when the map was never initiated (nothing confirmed yet, so the queue is
    * necessarily empty). Otherwise asserts that the freshly persisted digest agrees with
    * the in-memory confirmed digest, since the whole point of committing is that the two
    * must never diverge.
    */
  def commit(): Unit = {
...
object MempoolPlasmaMap {
  ...
  /** BatchAVLProver.topNode is `protected` at the Scala level (even though the generated
    * bytecode is public), so cloneInto's isNew-marking walk (see markWholeTreeNew) has to
    * reach it through reflection instead of a direct call.
    */
  ...
  /** Commits every live MempoolPlasmaMap of Paideia.current (see PaideiaSession.liveMaps)
    * and returns how many were committed. Intended to be called once per confirmed block,
    * after all of that block's transactions have been handled, so every confirmed
    * mutation queued during the block gets persisted in one pass. See
  ...
  /** Closes every live MempoolPlasmaMap's underlying LevelDB handle (see
    * MempoolPlasmaMap.close()) of Paideia.current, releasing every LOCK file so the same
    * directories can be reopened by fresh stores afterwards. See
