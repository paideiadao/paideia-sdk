...
class TotalStakingState(
  ...
  def stake(stakingKey: String, amount: Long): List[ContextVar] = {
    ...
    StakingContextVars
      .stake(stakingKey, stakeRecord, currentStakingState.stake(stakingKey, stakeRecord))
      .contextVars
  }
  ...
  def compound(batchSize: Int): List[ContextVar] = {
    if (this.snapshots.size < daoConfig[Long](ConfKeys.im_paideia_staking_emission_delay))
      ...
      kv._2.stake match {
        ...
        case _ => {
          ...
          (
            kv._1,
            StakeRecord(
              kv._2.stake + (snapshotStake * snapshot._3(0) / snapshotTotalStaked),
              kv._2.rewards.indices
                .map((i: Int) =>
                  kv._2.rewards(i) + (snapshotStake * snapshot
                    ._3(i + 1) / snapshotTotalStaked)
                )
                .toList
            )
          )
  ...
  def emit(currentTime: Long, tokensInPool: Long): List[ContextVar] = {
    ...
    profit(0) += Math.min(
      daoConfig[Long](ConfKeys.im_paideia_staking_emission_amount),
      tokensInPool - profit(0)
    )
    this.snapshots.enqueue(
      (this.currentStakingState.totalStaked, this.currentStakingState, List(profit: _*))
    )
    currentStakingState = currentStakingState.clone(nextEmission)
    profit = Array.fill(
      daoConfig.getArray[Object](ConfKeys.im_paideia_staking_profit_tokenids).size + 2
    )(0L)
...
object TotalStakingState {
  ...
  def apply(
    ...
    val currentState = StakingState(daoKey, nextEmission)
    val profitTokensSize =
      daoConfig.getArray[Array[Byte]](ConfKeys.im_paideia_staking_profit_tokenids).size
    val snapshots = Queue[(Long, StakingState, List[Long])](
      Range(0, daoConfig[Long](ConfKeys.im_paideia_staking_emission_delay).toInt).map(i =>
        (
          0L,
          currentState.clone(nextEmission - (i + 1)),
          List.fill(profitTokensSize + 2)(0L)
        )
      ): _*
    )
    val newState = new TotalStakingState(
      ...
      currentState,
      snapshots,
      Array.fill(profitTokensSize + 2)(0L),
      nextEmission
    )
