...
class DAOConfigKey(
  ...
  /** None-branch: resolve a name for a key built from raw hashed bytes (e.g. deserialized
    * out of a persisted AVL+ tree) - first from this session's dynamic registry, falling
    * back to the process-global static ConfKeys names (covers a session created after
    * ConfKeys already registered them but before this session's own knownKeys was seeded
    * from staticNames - see PaideiaSession). Some-branch: a key built BY NAME - register
    * it into the current session's knownKeys. Names are content-addressed (hash -> name
    * is a pure function of the name), so a key that's ALSO one of the static ConfKeys
    * names being registered here too (redundantly with DAOConfigKey.apply(s: String)'s
    * own staticNames.putIfAbsent) is harmless.
    */
...
object DAOConfigKey {
  ...
  /** Hash -> original key name for keys built from a fixed, well-known name (i.e. via the
    * single-arg apply below) - process-global rather than per-session, because ConfKeys
    * is an `object` whose ~100 `val`s construct named keys at class-init time,
    ...
    * session created after ConfKeys was touched sees the standard names in its knownKeys
    * map view too), and DAOConfigKey's own constructor falls back to this map for a
    * session created before that point. Populated by DAOConfigKey.apply(s: String) ONLY
    * (the two-arg apply(s, bytes), for dynamic per-proposal/per-action keys, and any
    * other named construction, register into the current session's knownKeys only - see
    * DAOConfigKey's own constructor). Concurrent because suites run in parallel and can
    * all touch ConfKeys at once.
    */
  ...
  /** Facade over Paideia.current.knownKeys (the per-session name registry - seeded from
    ...
    * DAOConfigKey construction since), kept for source compatibility with every existing
    * caller (paideia-state's getDAOConfig endpoint, persistState, tests, ...).
    */
