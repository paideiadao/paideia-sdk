...
case class ProtoDAOProxyBox(
    ...
    stakingProfitSharePct: Byte) extends PaideiaBox {
...
object ProtoDAOProxyBox {
    def fromInputBox(ctx: BlockchainContextImpl, inp: InputBox): ProtoDAOProxyBox = {
        ...
        val byteRegister = inp.getRegisters().get(0).getValue().asInstanceOf[Coll[Coll[Byte]]].map(_.toArray)
        val longRegister = inp.getRegisters().get(1).getValue().asInstanceOf[Coll[Long]]
        val stakePoolSize = longRegister(0)
        val daoName = DAOConfigValueDeserializer.deserialize(byteRegister(0).toArray).asInstanceOf[String]
        val daoTokenId = new ErgoId(DAOConfigValueDeserializer.deserialize(byteRegister(1).toArray).asInstanceOf[Array[Any]].map(_.asInstanceOf[Byte]))
