...
final case class CastVoteTransaction(
    ...
    val voteInput = Paideia.getBox(new FilterNode(
        FilterType.FTALL,
        List(
            new FilterLeaf(
                FilterType.FTEQ,
    ...
    val proposalInput = Paideia.getBox(new FilterLeaf(
                FilterType.FTEQ,
        ...
        (box: InputBox) =>
            box.getRegisters().get(0).getValue().asInstanceOf[Coll[Int]](0) == castVoteBox.proposalIndex
    )(0)
    ...
    val result = Paideia.getProposalContract(proposalInput).castVote(ctx,proposalInput,castVoteBox.vote,castVoteBox.voteKey)
    ...
    val extraContext = List(
        ContextVar.of(3.toByte,TotalStakingState(dao.key).currentStakingState.getStakes(List(voteBox.stakeKey)).proof.ergoValue),
    ...
    val voteOutput = voteBox.useContract.box(
        ...
        voteBox.voteKey,
        voteBox.stakeKey,
        voteBox.releaseTime.max(proposalInput.getRegisters().get(1).getValue().asInstanceOf[Coll[Long]](0)))

    val userOutput = _ctx.newTxBuilder().outBoxBuilder().tokens(
        new ErgoToken(castVoteBox.voteKey,1L)
    ).contract(castVoteBox.userAddress.toErgoContract).build()

    inputs = List(proposalInput.withContextVars(configContext++result._1++extraContext:_*),voteInput,castVoteInput)
